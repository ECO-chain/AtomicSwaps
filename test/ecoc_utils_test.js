...
describe("ECOC utilis tests", function() {
  ...
  it("should be equal", async function() {
    let addr = "eELfUyry7GkMVi7xNjq9zpuYbKbRsa6Hdg";
    let hex = "864f980d88cdfb44a2e3293ecc01084e6d686e7a";
    await ecoc_utils.getHexAddress(addr).then(results => {
      let computed_hex = results;
      assert.equal(computed_hex, hex);
    });
  });
  it("should be equal", async function() {
    let addr = "eRE1FUvaQdfCzHANAAcKoimPgBcxq2vMsN";
    let hex = "fdb69114c41081ba5af1a6074a4d6950739a77b0";
    await ecoc_utils.getHexAddress(addr).then(results => {
      let computed_hex = results;
      assert.equal(computed_hex, hex);
    });
